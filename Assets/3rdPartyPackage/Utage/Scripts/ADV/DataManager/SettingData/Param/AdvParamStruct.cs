	...
	public class AdvParamStruct
	{
		...
		public AdvParamStruct(AdvParamStruct header, StringGridRow values)
		{
			...
			foreach (var keyValue in header.Tbl)
			{
				...
				string value = values.Strings[index];
				AdvParamData data = new AdvParamData();
		...
		int ToIndexCommentOuted(StringGridRow row, int index)
		{
			int countCommentOut = 0;
			for ( int i = 0; i <= index; ++i)
			{
				if (row.Strings[i].StartsWith("//"))
				{
					++countCommentOut;
				}
			}
			return index + countCommentOut;
		}

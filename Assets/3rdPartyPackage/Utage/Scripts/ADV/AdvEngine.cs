...
namespace Utage
{
	...
	public partial class AdvEngine : MonoBehaviour
	{
        ...
        public void BootFromExportData(AdvImportScenarios scenarios, string resourceDir, string assetRootDir = "")
		{
			this.gameObject.SetActive(true);
			StopAllCoroutines();
			StartCoroutine(CoBootFromExportData(scenarios, resourceDir, assetRootDir));
		}
        ...
        /// <param name="assetRootDir">アセットバンドルロードする場合のプロジェクトルートディレクトリ.アセットバンドルの場合はここがリソースディレクトリルートになる.</param>
        IEnumerator CoBootFromExportData(AdvImportScenarios scenarios, string resourceDir, string assetRootDir = "")
		{
			ClearSub(false);
			isStarted = true;
			...
			BootInit(resourceDir, assetRootDir);
        ...
        /// <param name="assetRootDir">アセットバンドルロードする場合のプロジェクトルートディレクトリ.アセットバンドルの場合はここがリソースディレクトリルートになる.</param>
        void BootInit(string rootDirResource, string assetRootDir = "")
		{
			//カスタムコマンドの初期化
			BootInitCustomCommand();

			DataManager.BootInit(rootDirResource, assetRootDir);
			//設定データを反映
		...
		{
            Debug.Log("[AdvEngineStarter] StartEngine label=" + scenarioLabel);

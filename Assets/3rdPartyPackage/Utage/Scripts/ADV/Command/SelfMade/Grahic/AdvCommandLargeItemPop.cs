...
namespace Utage
{
	...
	/// <summary>
	/// 自作コマンド：大きいアイテムポップアップ表示   develop by ryo yoshida. 2018/08/07
	/// </summary>
	internal class AdvCommandLargeItemPop : AdvCommand
	{
		public AdvCommandLargeItemPop(StringGridRow row, AdvSettingDataManager dataManager)
			: base(row)
		{
			// アニメーション本体.
			if (!dataManager.OtherObjectSetting.Dictionary.ContainsKey("AdvItemLargePop")){
				Debug.LogError(ToErrorString("AdvItemPop is not contained in file setting"));
			}
			itemPopGraphic = dataManager.OtherObjectSetting.LabelToGraphic("AdvItemLargePop");
		...
		public override void DoCommand(AdvEngine engine)
		{
			...
			if(prefab == null){
				Debug.LogError("[AdvCommandLargeItemPop] DoCommand Error!! : object not setting.");
				return;
			}
        ...
        {
			engine.GraphicManager.DrawObject(layer, "AdvItemLargePop", this.graphicOperaitonArg);
			var obj = engine.GraphicManager.SpriteManager.FindObject("AdvItemLargePop");
            while (obj == null) {
                yield return null;
				obj = engine.GraphicManager.SpriteManager.FindObject("AdvItemLargePop");
            }

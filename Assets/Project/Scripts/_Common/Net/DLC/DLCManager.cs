...
public static class DLCManager
{
    ...
    static DLCManager()
    {
        ...
        DLCFolderPaths[(int)DLC_FOLDER.ScreenBG] = s3_asset_screenbg_path;
        var s3_asset_icon_path = "Icon/" + platform;
        DLCFolderPaths[(int)DLC_FOLDER.Icon] = s3_asset_icon_path;
    ...
    public static void Init()
    {
        ...
        SendAPI.FilelistIndex ((sucess, result) => {
            ...
            var url = string.Format("{0}FileList/{1}?v={2}{3}", ClientDefine.URL_DLC, fileName, FileListHash, result.Key.ToHashMD5());
#if DEBUG
    ...
    public static void AssetBundleFromFileOrDownload(DLC_FOLDER folderType, string fileName, AssetBundleLoadRequest.OnSuccess didLoad, FileDownloadRequest.OnError didError = null, FileDownloadRequest.OnProgress progress = null)
    {
        var path = GetDownloadPath (folderType, fileName);

...
{
	[MessagePackFormatter(typeof(ReceiveUsersUnfollowUserFormatter))]
	public partial class ReceiveUsersUnfollowUser : BaseReceiveAPI
	{
		// response
		public UserData UserData;

		class ReceiveUsersUnfollowUserFormatter : IMessagePackFormatter<ReceiveUsersUnfollowUser>
		{
			public int Serialize(ref byte[] bytes, int offset, ReceiveUsersUnfollowUser value, IFormatterResolver formatterResolver) {
				if (value == null) {
			...
			public ReceiveUsersUnfollowUser Deserialize(byte[] bytes, int offset, IFormatterResolver formatterResolver, out int readSize) {
				...
				int readed = 0;
				var startOffset = offset;
				ReceiveUsersUnfollowUser ret = new ReceiveUsersUnfollowUser();

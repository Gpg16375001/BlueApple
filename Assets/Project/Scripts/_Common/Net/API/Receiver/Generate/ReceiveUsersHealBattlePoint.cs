...
{
	[MessagePackFormatter(typeof(ReceiveUsersHealBattlePointFormatter))]
	public partial class ReceiveUsersHealBattlePoint : BaseReceiveAPI
	{
		// response
		public PvpUserData PvpUserData;
		public UserData UserData;

		class ReceiveUsersHealBattlePointFormatter : IMessagePackFormatter<ReceiveUsersHealBattlePoint>
		{
			public int Serialize(ref byte[] bytes, int offset, ReceiveUsersHealBattlePoint value, IFormatterResolver formatterResolver) {
				if (value == null) {
			...
			public ReceiveUsersHealBattlePoint Deserialize(byte[] bytes, int offset, IFormatterResolver formatterResolver, out int readSize) {
				...
				int readed = 0;
				var startOffset = offset;
				ReceiveUsersHealBattlePoint ret = new ReceiveUsersHealBattlePoint();

...
{
	[MessagePackFormatter(typeof(ReceiveWeaponsSellWeaponFormatter))]
	public partial class ReceiveWeaponsSellWeapon : BaseReceiveAPI
	{
		// response
		public UserData UserData;

		class ReceiveWeaponsSellWeaponFormatter : IMessagePackFormatter<ReceiveWeaponsSellWeapon>
		{
			public int Serialize(ref byte[] bytes, int offset, ReceiveWeaponsSellWeapon value, IFormatterResolver formatterResolver) {
				if (value == null) {
			...
			public ReceiveWeaponsSellWeapon Deserialize(byte[] bytes, int offset, IFormatterResolver formatterResolver, out int readSize) {
				...
				int readed = 0;
				var startOffset = offset;
				ReceiveWeaponsSellWeapon ret = new ReceiveWeaponsSellWeapon();

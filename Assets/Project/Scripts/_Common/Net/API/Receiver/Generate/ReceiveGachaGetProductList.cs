...
{
	[MessagePackFormatter(typeof(ReceiveGachaGetProductListFormatter))]
	public partial class ReceiveGachaGetProductList : BaseReceiveAPI
	{
		// response
		public GachaProductData[] GachaProductDataList;
		public GachaData[] GachaDataList;
		public UserData UserData;
		public int RarestCardMissGachaTriggerCount;
		public int RarestCardMissCount;

		class ReceiveGachaGetProductListFormatter : IMessagePackFormatter<ReceiveGachaGetProductList>
		{
			public int Serialize(ref byte[] bytes, int offset, ReceiveGachaGetProductList value, IFormatterResolver formatterResolver) {
				if (value == null) {
				...
				offset += MessagePackBinary.WriteString (ref bytes, offset, "GachaProductDataList");
				if(value.GachaProductDataList == null) {
					offset += MessagePackBinary.WriteNil (ref bytes, offset);
				} else {
					var count = value.GachaProductDataList.Length;
					offset += MessagePackBinary.WriteArrayHeader(ref bytes, offset, count);
					for(int i = 0; i < count; ++i) {
						if(value.GachaProductDataList[i] == null) {
							offset += MessagePackBinary.WriteNil (ref bytes, offset);
						} else {
							offset += formatterResolver.GetFormatter<GachaProductData> ().Serialize (ref bytes, offset, value.GachaProductDataList[i], formatterResolver);
						}
					}
				}
				offset += MessagePackBinary.WriteString (ref bytes, offset, "GachaDataList");
				if(value.GachaDataList == null) {
					offset += MessagePackBinary.WriteNil (ref bytes, offset);
				} else {
					var count = value.GachaDataList.Length;
					offset += MessagePackBinary.WriteArrayHeader(ref bytes, offset, count);
					for(int i = 0; i < count; ++i) {
						if(value.GachaDataList[i] == null) {
							offset += MessagePackBinary.WriteNil (ref bytes, offset);
						} else {
							offset += formatterResolver.GetFormatter<GachaData> ().Serialize (ref bytes, offset, value.GachaDataList[i], formatterResolver);
						}
					}
				}
			...
			public ReceiveGachaGetProductList Deserialize(byte[] bytes, int offset, IFormatterResolver formatterResolver, out int readSize) {
				...
				int readed = 0;
				var startOffset = offset;
				ReceiveGachaGetProductList ret = new ReceiveGachaGetProductList();
				...
				for (int i = 0; i < headerCount; ++i) {
					...
					if (key == "GachaProductDataList") {
						if (MessagePackBinary.IsNil (bytes, offset)) {
							offset += 1;
							ret.GachaProductDataList = null;
						} else {
							var count = MessagePackBinary.ReadArrayHeader(bytes, offset, out readed);
							offset += readed;
							ret.GachaProductDataList = new GachaProductData[count];
							for(int arrayIndex = 0; arrayIndex < count; ++arrayIndex) {
								if (MessagePackBinary.IsNil (bytes, offset)) {
									offset += 1;
									ret.GachaProductDataList[arrayIndex] = null;
								} else {
									ret.GachaProductDataList[arrayIndex] = formatterResolver.GetFormatter<GachaProductData> ().Deserialize (bytes, offset, formatterResolver, out readed);
									offset += readed;
								}
							}
						}
					...
					if (key == "GachaDataList") {
						if (MessagePackBinary.IsNil (bytes, offset)) {
							offset += 1;
							ret.GachaDataList = null;
						} else {
							var count = MessagePackBinary.ReadArrayHeader(bytes, offset, out readed);
							offset += readed;
							ret.GachaDataList = new GachaData[count];
							for(int arrayIndex = 0; arrayIndex < count; ++arrayIndex) {
								if (MessagePackBinary.IsNil (bytes, offset)) {
									offset += 1;
									ret.GachaDataList[arrayIndex] = null;
								} else {
									ret.GachaDataList[arrayIndex] = formatterResolver.GetFormatter<GachaData> ().Deserialize (bytes, offset, formatterResolver, out readed);
									offset += readed;
								}
							}
						}

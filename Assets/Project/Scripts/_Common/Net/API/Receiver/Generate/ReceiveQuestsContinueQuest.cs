...
{
	[MessagePackFormatter(typeof(ReceiveQuestsContinueQuestFormatter))]
	public partial class ReceiveQuestsContinueQuest : BaseReceiveAPI
	{
		// response
		public UserData UserData;

		class ReceiveQuestsContinueQuestFormatter : IMessagePackFormatter<ReceiveQuestsContinueQuest>
		{
			public int Serialize(ref byte[] bytes, int offset, ReceiveQuestsContinueQuest value, IFormatterResolver formatterResolver) {
				if (value == null) {
			...
			public ReceiveQuestsContinueQuest Deserialize(byte[] bytes, int offset, IFormatterResolver formatterResolver, out int readSize) {
				...
				int readed = 0;
				var startOffset = offset;
				ReceiveQuestsContinueQuest ret = new ReceiveQuestsContinueQuest();

...
{
	[MessagePackFormatter(typeof(ReceiveUsersGetFriendCandidateListFormatter))]
	public partial class ReceiveUsersGetFriendCandidateList : BaseReceiveAPI
	{
		// response
		public UserData[] UserDataList;

		class ReceiveUsersGetFriendCandidateListFormatter : IMessagePackFormatter<ReceiveUsersGetFriendCandidateList>
		{
			public int Serialize(ref byte[] bytes, int offset, ReceiveUsersGetFriendCandidateList value, IFormatterResolver formatterResolver) {
				if (value == null) {
			...
			public ReceiveUsersGetFriendCandidateList Deserialize(byte[] bytes, int offset, IFormatterResolver formatterResolver, out int readSize) {
				...
				int readed = 0;
				var startOffset = offset;
				ReceiveUsersGetFriendCandidateList ret = new ReceiveUsersGetFriendCandidateList();

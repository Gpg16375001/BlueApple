...
{
	[MessagePackFormatter(typeof(ReceivePaymentsSubmitReceiptFormatter))]
	public partial class ReceivePaymentsSubmitReceipt : BaseReceiveAPI
	{
		// response
		public UserData UserData;

		class ReceivePaymentsSubmitReceiptFormatter : IMessagePackFormatter<ReceivePaymentsSubmitReceipt>
		{
			public int Serialize(ref byte[] bytes, int offset, ReceivePaymentsSubmitReceipt value, IFormatterResolver formatterResolver) {
				if (value == null) {
			...
			public ReceivePaymentsSubmitReceipt Deserialize(byte[] bytes, int offset, IFormatterResolver formatterResolver, out int readSize) {
				...
				int readed = 0;
				var startOffset = offset;
				ReceivePaymentsSubmitReceipt ret = new ReceivePaymentsSubmitReceipt();

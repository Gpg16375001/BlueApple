...
{
	[MessagePackFormatter(typeof(ReceivePvpUpdateOpponentListFormatter))]
	public partial class ReceivePvpUpdateOpponentList : BaseReceiveAPI
	{
		// response
		...
		class ReceivePvpUpdateOpponentListFormatter : IMessagePackFormatter<ReceivePvpUpdateOpponentList>
		{
			public int Serialize(ref byte[] bytes, int offset, ReceivePvpUpdateOpponentList value, IFormatterResolver formatterResolver) {
				if (value == null) {
			...
			public ReceivePvpUpdateOpponentList Deserialize(byte[] bytes, int offset, IFormatterResolver formatterResolver, out int readSize) {
				...
				int readed = 0;
				var startOffset = offset;
				ReceivePvpUpdateOpponentList ret = new ReceivePvpUpdateOpponentList();

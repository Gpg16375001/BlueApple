...
{
	[MessagePackFormatter(typeof(ReceiveBattlesOpenBattleStageFormatter))]
	public partial class ReceiveBattlesOpenBattleStage : BaseReceiveAPI
	{
		// response
		public BattleEntryData BattleEntryData;

		class ReceiveBattlesOpenBattleStageFormatter : IMessagePackFormatter<ReceiveBattlesOpenBattleStage>
		{
			public int Serialize(ref byte[] bytes, int offset, ReceiveBattlesOpenBattleStage value, IFormatterResolver formatterResolver) {
				if (value == null) {
			...
			public ReceiveBattlesOpenBattleStage Deserialize(byte[] bytes, int offset, IFormatterResolver formatterResolver, out int readSize) {
				...
				int readed = 0;
				var startOffset = offset;
				ReceiveBattlesOpenBattleStage ret = new ReceiveBattlesOpenBattleStage();

...
{
	[MessagePackFormatter(typeof(ReceiveUsersHealActionPointFormatter))]
	public partial class ReceiveUsersHealActionPoint : BaseReceiveAPI
	{
		// response
		public UserData UserData;

		class ReceiveUsersHealActionPointFormatter : IMessagePackFormatter<ReceiveUsersHealActionPoint>
		{
			public int Serialize(ref byte[] bytes, int offset, ReceiveUsersHealActionPoint value, IFormatterResolver formatterResolver) {
				if (value == null) {
			...
			public ReceiveUsersHealActionPoint Deserialize(byte[] bytes, int offset, IFormatterResolver formatterResolver, out int readSize) {
				...
				int readed = 0;
				var startOffset = offset;
				ReceiveUsersHealActionPoint ret = new ReceiveUsersHealActionPoint();

...
{
	[MessagePackFormatter(typeof(ReceiveTeamsGetCardListFormatter))]
	public partial class ReceiveTeamsGetCardList : BaseReceiveAPI
	{
		// response
		public CardData[] CardDataList;

		class ReceiveTeamsGetCardListFormatter : IMessagePackFormatter<ReceiveTeamsGetCardList>
		{
			public int Serialize(ref byte[] bytes, int offset, ReceiveTeamsGetCardList value, IFormatterResolver formatterResolver) {
				if (value == null) {
			...
			public ReceiveTeamsGetCardList Deserialize(byte[] bytes, int offset, IFormatterResolver formatterResolver, out int readSize) {
				...
				int readed = 0;
				var startOffset = offset;
				ReceiveTeamsGetCardList ret = new ReceiveTeamsGetCardList();

...
{
	[Serializable]
	[MessagePackFormatter(typeof(PvpCardDataFormatter))]
	public partial class PvpCardData
	{
		...
		class PvpCardDataFormatter : IMessagePackFormatter<PvpCardData>
		{
			public int Serialize(ref byte[] bytes, int offset, PvpCardData value, IFormatterResolver formatterResolver) {
				if (value == null) {
			...
			public PvpCardData Deserialize(byte[] bytes, int offset, IFormatterResolver formatterResolver, out int readSize) {
				...
				int readed = 0;
				var startOffset = offset;
				PvpCardData ret = new PvpCardData();

...
public class ListItem_Option_ReceiptHistory : ViewBase
{
	...
	public void UpdateData(DistributionData data)
	{
		...
		this.GetScript<TextMeshProUGUI>("txtp_Num").text = itemType == ItemTypeEnum.paid_gem || itemType == ItemTypeEnum.free_gem || itemType == ItemTypeEnum.money ? data.Quantity.ToString("#,0"): data.Quantity.ToString();
